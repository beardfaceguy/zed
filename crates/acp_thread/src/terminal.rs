...
pub fn persistent_sandbox_wrap(
    ...
    sandbox_wrap_from_permissions(
        &AgentSettings::get_global(cx).sandbox_permissions,
        project
            .worktrees(cx)
            .map(|worktree| worktree.read(cx).abs_path().to_path_buf())
            .collect(),
        external_agent_sandbox_git_dirs(project, cx),
        wsl_zed_release,
    )
}

fn external_agent_sandbox_git_dirs(project: &Project, cx: &App) -> Vec<PathBuf> {
    let mut git_dirs = Vec::new();
    for worktree in project.worktrees(cx) {
        let worktree = worktree.read(cx);
        git_dirs.push(worktree.abs_path().join(".git"));
...
#[cfg(test)]
mod external_agent_sandbox_tests {
    use super::*;
    ...
    fn persistent_unsandboxed_setting_disables_wrap() {
        ...
        assert!(
            sandbox_wrap_from_permissions(&permissions, vec![], vec![], None)
                .unwrap()
                .is_none()
        );
    }
    ...
    #[test]
    fn persistent_write_grants_are_preserved() {
        let grant = settings::GrantedWritePath::from_requested(PathBuf::from("/approved"));
        let permissions = SandboxPermissions {
            write_paths: vec![grant.clone()],
            ..Default::default()
        };
        ...
        assert_eq!(wrap.extra_write_paths, vec![grant]);
    ...
    fn persistent_network_hosts_build_restricted_allowlist() {
        ...
        let wrap = sandbox_wrap_from_permissions(&permissions, vec![], vec![], None)
            .unwrap()
            .unwrap();

        let SandboxNetworkAccess::Restricted(allowlist) = wrap.network else {
            panic!("expected restricted network access");
        };
